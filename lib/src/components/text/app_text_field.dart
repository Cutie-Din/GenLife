...
class _AppTextFieldState extends State<AppTextField> {
  ...
  Widget? get _suffixIcon => GestureDetector(
    ...
    child: Padding(padding: const EdgeInsets.all(0), child: widget.suffixIcon),
  );
  ...
  BoxBorder get _border => widget.border ?? Border.all(color: AppColors.black);
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        if ((widget.title ?? '').isNotEmpty) ...[Text(widget.title ?? ''), const Gap(7)],
        SizedBox(
          height: _height + 20,
          width: 327,
          child: TextFormField(
