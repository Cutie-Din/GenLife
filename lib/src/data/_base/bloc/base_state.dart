...
class BaseState<T> {
  ...
  BaseState copyWith({BaseStatus? status, String? message}) {
    return BaseState(status: status ?? this.status, message: message ?? this.message);
  }

  factory BaseState.fromJson(Map<String, dynamic> json, T Function(Object? json) fromJsonT) =>
